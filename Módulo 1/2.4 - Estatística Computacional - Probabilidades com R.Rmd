---
title: "2.4 - Estatística Computacional - Probabilidades com R"
...
# A probabilidade de até dois clientes comprarem é:(probabilidade de nenhum cliente comprar) + (probabilidade de um cliente comprar) + probabilidade de dois cliente comprarem)
...
plot(va_poison)
# Observe que os valores se distribuiem simetricamente en tormo de seis, use acontece porque o paramentro lambda é a média (e também o desvio padrão) da distribuição de Poisson
``` 
```{r}
...
# P(X<=2600) - P(X<=2400)
 probabilidade_2400_a_2600 <- (probabilidade_ate_2600 - probabilidade_ate_2400)
 probabilidade_2400_a_2600
```
...
```
                          ...
                          #########################################
```{r}
# Gerando uma amostra aleatória de 1000 números seguindo uma distribuição F
...
# Vá aumentando o grau de liberdade e observando o comportamento do histograma (se aproxima da normal)
vb_t <- rt(1000, df = 10)
hist(vb_t)
```
